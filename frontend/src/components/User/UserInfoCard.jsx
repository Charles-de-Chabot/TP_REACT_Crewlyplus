...
const UserInfoCard = ({ userData, onEdit }) => {
    ...
    return (
        <div className="bg-slate-900/40 backdrop-blur-md border border-white/10 rounded-[2.5rem] p-8 shadow-xl shadow-black/30 sticky top-28 overflow-hidden group">
            {/* Background Glow */}
            ...
            <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter mb-8 flex items-center gap-3">
                <span className="w-8 h-8 rounded-xl bg-teal-500/20 text-teal-500 flex items-center justify-center text-sm not-italic font-black">i</span>
                Mes Infos
            </h3>
            ...
            <button 
                onClick={onEdit}
                className="w-full mt-12 py-4 bg-white/5 border border-white/10 rounded-2xl text-white font-black text-xs uppercase hover:bg-white/10 hover:border-teal-500/30 hover:text-teal-400 transition-all duration-300 flex items-center justify-center gap-2 group/btn"
            >
                <span>Modifier mon profil</span>
                <span className="opacity-0 group-hover/btn:opacity-100 transition-opacity">✏️</span>
            </button>
        </div>
    );
};
